...
if __name__ == '__main__':
    ...
    model = YoloV5(yolov5_yaml_path='config/yolov5s_2.yaml')
    ...
    try:
        while True:
            ...
            cv2.circle(canvas, (center_x, center_y), 10, (0, 0, 255), -1)
            ...
            if camera_xyz_list:
                for camera_xyz in camera_xyz_list:
                    x, y = camera_xyz[0], camera_xyz[1]
                    angle = math.degrees(math.atan2(y, x))
                    angle_text = f"Angle: {angle:.2f} degrees"
                    cv2.putText(canvas, angle_text, (canvas.shape[1] - 300, 50),
